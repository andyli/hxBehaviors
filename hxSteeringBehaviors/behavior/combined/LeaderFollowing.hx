package hxSteeringBehaviors.behavior.combined; 
	...
	import hxSteeringBehaviors.Float3DTools;
	...
	class LeaderFollowing extends AbstractBehavior {
		...
		public var vehicleList:Iterable<Vehicle>;
		public var vehicleLeader:Vehicle;
		
		public function new(?vehicleList:Iterable<Vehicle>, ?vehicleLeader:Vehicle)
		{
			super();
		...
		public function steeringForLeader(veh:Vehicle, v:Float3D):Void
		{
			...
			if(v.equalsZero())
			{
				if(Float3DTools.random() < 0.8)
				{
					var r:Float = Float3DTools.random();
		...
		public function maybeSteerTowardRandomLeaderTarget(veh:Vehicle, v:Float3D):Void
		{
			if(leaderTargetCount == 0 || leaderTarget.approximateDistance(veh.position) < veh.vehicleRadius * 3)
			{
				...
				do
				{
					leaderTarget.x = leaderTargetRange * (Float3DTools.random() * 0.8 + 0.1);
					leaderTarget.y = leaderTargetRange * (Float3DTools.random() * 0.8 + 0.1);
					leaderTarget.z = leaderTargetRange * (Float3DTools.random() * 0.8 + 0.1);
